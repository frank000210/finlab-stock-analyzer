...
| LLM API | OpenCode Go（`https://opencode.ai/zen/go/v1`，OpenAI 相容端點），跟主站 `backend/app/llm/client.py` 共用同一把金鑰，於 Zeabur 服務環境變數 `OPENCODE_API_KEY` 設定 | 原規劃用 Claude API，但使用者提供的金鑰實際是 OpenCode Go（見 `backend/app/config/settings.py` 的 `opencode_api_key`），改走跟主站相同、已驗證過的整合方式 |
...
| 問答生成 | OpenCode Go（`httpx` 呼叫 OpenAI 相容 `/chat/completions`，主模型 `minimax-m2.5`，空內容時換備援模型 `qwen3.7-plus`） |
...
| 3. 呼叫 OpenCode Go | 用環境變數 `OPENCODE_API_KEY`；需帶識別用 User-Agent（Cloudflare 會擋預設 UA）；回應內容為空時自動換備援模型重試一次；失敗/超時回傳明確錯誤，不讓前端卡住 |
...
使用者登入後，先在「Notebook 總覽」看目前有哪些領域知識、各有多少篇；到「資料匯入」頁拖檔案或貼網址把新資料加進指定 domain；到「問答」頁針對某個 domain（或全部）提問，後端檢索相關片段餵給 OpenCode Go，生成有引用來源的答案。
...
- **安全**：密碼、`OPENCODE_API_KEY`、MongoDB 連線字串一律走環境變數，不寫死在程式碼、不入庫；登入失敗需有基本延遲防暴力破解。
...
- **環境變數**（於 Zeabur 服務設定，不經本對話）：`MONGODB_URI`、`OPENCODE_API_KEY`、`KB_WEB_PASSWORD`、`KB_API_TOKEN`。
