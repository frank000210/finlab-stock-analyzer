...
test('風控監控 尚無交易日誌紀錄時顯示空狀態與 ACTIVE (F7)', async ({ page }) => {
  ...
  await expect(page.locator('.status-pill')).toHaveText('正常')
...
test('風控監控 實際回撤達 6% 時顯示 WARNING (F7)', async ({ page }) => {
  ...
  await expect(page.locator('.status-pill')).toHaveText('警戒')
})
...
test('風控監控 實際回撤達 10% 以上時顯示 PAUSED (F7)', async ({ page }) => {
  ...
  await expect(page.locator('.status-pill')).toHaveText('暫停')
})
