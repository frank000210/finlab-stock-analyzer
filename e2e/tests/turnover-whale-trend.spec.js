  ...
  await page.route('**/api/v1/analysis/2330/turnover', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        success: true,
        data: {
          symbol: '2330', turnover_pct: 0.85, percentile: 92, cap_tier: '大型/中型',
          series: [{ date: '2026-07-15', turnover_pct: 0.85 }],
          as_of: '2026-07-15', source: 'finmind',
        },
      }),
    })
  })
...
test('分析頁顯示千張大戶持股趨勢，三重訊號同向時給聯合判讀 (Q2/Q4)', async ({ page }) => {
  ...
  // 換手率也放大，且股價當日上漲 → 應觸發三重訊號聯合判讀
  await page.route('**/api/v1/analysis/2330/turnover', async (route) => {
    await route.fulfill({
      status: 200, contentType: 'application/json',
      body: JSON.stringify({
        success: true,
        data: { symbol: '2330', turnover_pct: 0.9, percentile: 88, cap_tier: '大型/中型', series: [], as_of: '2026-07-15', source: 'finmind' },
      }),
    })
  })
