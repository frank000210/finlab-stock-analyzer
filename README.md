...
- 📊 **基本面**：月營收、EPS、獲利率、負債比，含本益比河流圖（PE/PB Band）與營收成長圖，並提供同業比較（自動抓同產業標的，亦可 AI 建議或手動加入，目標股於估值/成長指標中高亮定位）
- 🏦 **籌碼面**：三大法人買賣超、持股比、融資融券、主力成本、大戶/散戶持股結構、千張大戶持股週度趨勢與換手率異常放大偵測
- 🕸️ **觀察股關聯圖**：力導向圖／階層邊綁定／相關矩陣熱力圖三種檢視，追蹤標的間的領先落後與資金流向
- 🚦 **大盤多空儀表板**：三燈號（趨勢/寬度/風險）綜合市場體制判讀，獨立頁面速覽整體多空氛圍
...
- 🔄 **策略回測**：FinLab 風格回測引擎，內建多種策略，含成本試算、MFE/MAE 分析、樣本外驗證；亦可用「自訂條件」模式自行組合 指標(參數) 比較符 指標(參數) 的買賣觸發條件（類似簡化版 Pine Script）
...
**AI（選填，需設定 LLM 金鑰）**
- 🤖 **個股 AI 摘要**：把分析頁已算好的數據串成白話摘要，指出數據間矛盾
- 🤖 **AI 選同業**：一鍵憑 LLM 知識建議真同業（代碼皆經真實名單驗證），取代手動複製提示詞去 Gemini 的流程
- ⚠️ **財報矛盾偵測**：純規則、免 LLM 成本，自動標出「營收成長但 EPS 衰退」等異常組合
- 🤖 **輿情摘要**：把 PTT/新聞/財經媒體標題總結成多空判讀
- 🔍 **新聞可信度 AI 語意層**：五層規則式評分（媒體來源/Cofacts/內容特徵/交叉驗證/時效）＋選填 AI 語意判讀
- 🎓 **AI 複盤**：分析交易日誌找出規則式教練沒設計到的細緻行為模式；進場理由品質檢查
- 🔎 **AI 選股**：自然語言描述條件，LLM 解析成篩選門檻，套用網站既有數據跑數字比較（不會幫你編造股票）
- 🔎 **AI 生成回測條件**：白話描述進出場想法，LLM 翻譯成回測頁「自訂條件」模式看得懂的運算式（固定語法白名單解析，不執行 LLM 生成的程式碼），生成後仍可自行編輯再送出
- 📋 **盤後日報自然語言化**：LLM 把結構化資料改寫成播報稿，失敗自動退回原本模板
...
回測頁另有「自訂條件」模式，不是固定範本，而是用 指標(參數) 比較符 指標(參數)（可 AND/OR/NOT 組合、AI 生成或自行輸入）自訂買賣觸發條件，語法規則見 `backend/app/backtest/expr_lang.py`。
