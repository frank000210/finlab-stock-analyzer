...
async def _build_chip_analysis(symbol: str, days: int) -> dict:
    ...
    # 籌碼健診評分 (6 面向加權綜合)
    result["chip_health"] = compute_chip_health(
        result["distribution"], result["major_players"], result["major_cost"],
        result["sync_buy"], result["margin_ratio"], result["day_trade"],
    )
...
@router.get("/{symbol}/chip-score")
async def get_chip_score(
    symbol: str,
    days: int = Query(default=90, ge=20, le=365),
):
    """輕量籌碼健診評分：供個股分析頁顯示綜合籌碼分數 (複用完整分析快取)."""
    try:
        result = await _build_chip_analysis(symbol, days)
